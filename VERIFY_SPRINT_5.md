...
VERIFY is v1.0 complete. Sprint 5 focuses on three things that
...
```bash
# Add the crate
cargo add fasttext-pure-rs -p verify-classifier

# Run the diagnostic probe
cargo run --example lid_label_probe \
    --features verify-classifier/fasttext-probe \
    -p verify-classifier 2>&1
```
...
(free account). This is the first VERIFY feature that requires
an HF token. Handle carefully — the token must be stored in
`~/.cache/verify/hf_token` (not hardcoded, not in env where
it could leak).
...
impl HfTokenManager {
    ...
    pub fn load(&self) -> Result<String, VerifyError> {
...
```bash
verify setup --hf-token hf_xxxxx
# Writes token to ~/.cache/verify/hf_token
# Confirms by attempting a test API call (online check)
```

**Step 2 — Diarization worker script**

`crates/verify-stt/scripts/diarize.py`:
...
impl DiarizationEngine {
    ...
    pub fn diarize(
        &self,
        audio_path: &Path,
    ) -> Result<Vec<DiarizationSegment>, VerifyError>;
}
```
...
verify translate --input interview.mp3 --target en --diarize

# Without (default — no HF account needed)
verify translate --input interview.mp3 --target en
```
...
fn hf_token_manager_returns_clear_error_when_missing() {
    // No token file → VerifyError with setup instructions
}
```

### Acceptance criteria — P2

- [ ] `verify setup --hf-token` stores token securely
...
the internet at all. VERIFY's download-on-first-run model doesn't
...
```bash
#!/bin/bash
# Build a VERIFY air-gap package with all model weights
# Run this on an internet-connected machine, transfer to air-gapped workstation

set -e
OUTPUT="verify-airgap-$(date +%Y%m%d).tar.gz"
STAGING=$(mktemp -d)

echo "→ Downloading VERIFY model weights..."
...
#!/bin/bash
# Install VERIFY model weights on air-gapped workstation
CACHE="$HOME/.cache/verify/models"
mkdir -p "$CACHE/whisper"
cp lid.176.ftz "$CACHE/"
cp whisper-*.bin "$CACHE/whisper/"
cp -r nllb-600M "$CACHE/nllb/"
echo "✅ VERIFY models installed. Run: verify classify --text 'test'"
INSTALL
...
When VERIFY runs and the model isn't in cache, before trying
to download, check for a `VERIFY_AIRGAP_PATH` environment variable:
...
pub fn ensure_lid_model(&self) -> Result<PathBuf, VerifyError> {
    ...
    if let Ok(airgap) = std::env::var("VERIFY_AIRGAP_PATH") {
...
tar -xzf verify-airgap-YYYYMMDD.tar.gz
bash install.sh

## Verify installation:
VERIFY_AIRGAP_PATH=~/.cache/verify/models verify classify \
...
fn model_manager_uses_airgap_path_when_set() {
    // Set VERIFY_AIRGAP_PATH env var to a temp dir with fake model
...
- [ ] `VERIFY_AIRGAP_PATH` env var respected by ModelManager
...
```
feat: verify-sprint-5-P1 fasttext-pure-rs — [compatible/incompatible]
feat: verify-sprint-5-P2 speaker diarization — pyannote, enriched segments
feat: verify-sprint-5-P3 air-gap package — offline installer + AIRGAP_PATH
```
...
_P3 makes VERIFY deployable in classified environments._
