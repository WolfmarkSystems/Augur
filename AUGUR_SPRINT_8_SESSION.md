# AUGUR Sprint 8 — 2026-04-26 — Session log
...
- Built a minimal `vendor/strata-fs/` stub crate exposing just
  ...
  re-exports + invokes. AUGUR's plugin walks `root_path`
...
- Workspace `Cargo.toml` adds `[workspace.exclude]` for both
  ...
  `crates/augur-plugin-sdk/Cargo.toml`.
...
- New CLI flag `augur batch --all-foreign` plumbed through
...
- New `SPEAKER_DIARIZATION_ADVISORY` const in
  `augur-stt::diarize`. Non-suppressible at the same level as
...
- **`--features augur-plugin-sdk/strata` build**: 119 unit
...
- The vendored SDK's dev-dep `tempfile = { workspace = true }`
  ...
  runs from AUGUR, so this is safe.
