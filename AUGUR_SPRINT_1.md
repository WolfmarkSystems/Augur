# AUGUR — Foundation Sprint 1
# Scaffold + fastText Classifier + Whisper STT

_Date: 2026-04-24_
_Model: claude-opus-4-7_
_Approved by: KR_
_Workspace: ~/Wolfmark/augur/ (greenfield — does not exist yet)_
...
AUGUR is offline-first by design. This is non-negotiable.
...
### Goal
Create the AUGUR Rust workspace at `~/Wolfmark/augur/` with the
...
├── crates/
│   ├── augur-core/              ← pipeline orchestrator
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── pipeline.rs       ← main pipeline entry point
│   │       └── error.rs          ← unified error type
│   ├── augur-classifier/        ← fastText language ID
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       └── classifier.rs
│   ├── augur-stt/               ← Whisper STT
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       └── whisper.rs
│   ├── augur-translate/         ← NLLB-200 (stub in Sprint 1)
│   │   ├── Cargo.toml
│   │   └── src/lib.rs
│   ├── augur-ocr/               ← Tesseract (stub in Sprint 1)
│   │   ├── Cargo.toml
│   │   └── src/lib.rs
│   └── augur-plugin-sdk/        ← Strata plugin adapter (stub in Sprint 1)
│       ├── Cargo.toml
│       └── src/lib.rs
├── apps/
│   └── augur-cli/               ← standalone binary
│       ├── Cargo.toml
│       └── src/
│           └── main.rs
└── tests/
...
### CLAUDE.md content for AUGUR workspace

Write `~/Wolfmark/augur/CLAUDE.md` with:
...
- [ ] `cargo build --workspace` succeeds from `~/Wolfmark/augur/`
...
- [ ] Git initialized, first commit: `chore: scaffold AUGUR workspace`
...
for AUGUR to process everything — the classifier scans fast, flags
...
In `crates/augur-classifier/Cargo.toml`:
```toml
[dependencies]
...
In `augur-classifier/src/classifier.rs`, write a `ModelManager` struct:
...
impl ModelManager {
    ...
    /// This is the ONLY network call AUGUR makes, and only on first run.
...
Cache directory: `~/.cache/augur/models/` (XDG-compliant)
...
impl LanguageClassifier {
    pub fn load(model_path: &Path) -> Result<Self, AugurError>;
...
Whisper converts audio content to text. AUGUR then passes the
...
In `crates/augur-stt/Cargo.toml`:
```toml
[dependencies]
...
```rust
fn preprocess_audio(input: &Path, output: &Path) -> Result<(), AugurError>
```
...
`AUGUR_RUN_INTEGRATION_TESTS=1` is set.
...
augur transcribe --input recording.wav --preset fast

# Full translate (stubs NLLB in Sprint 1, shows "TRANSLATION_STUB")
augur translate --input audio.mp3 --target en --preset balanced
```

### Implementation

In `apps/augur-cli/src/main.rs`, use `clap` for argument parsing:

```toml
[dependencies]
clap = { version = "4", features = ["derive"] }
augur-core = { path = "../../crates/augur-core" }
log = "0.4"
env_logger = "0.11"
```
...
[AUGUR] Transcript: مرحبا بالعالم
[AUGUR] Translation: STUB — NLLB-200 integration coming in Sprint 2
```

### Acceptance criteria — P4

- [ ] `cargo build --release -p augur-cli` produces a binary
- [ ] `augur classify --text "مرحبا بالعالم" --target en` runs
- [ ] `augur transcribe --input <wav> --preset fast` runs
- [ ] `augur --help` shows clean usage
...
```
chore: scaffold AUGUR workspace — 6 crates, CLI app, CLAUDE.md
feat: augur-classifier — fastText LID, ModelManager, 3 unit tests
feat: augur-stt — Whisper STT engine, 3 presets, audio preprocessing
feat: augur-cli — classify + transcribe + translate stub subcommands
```
...
- augur-plugin-sdk wired to Strata plugin API
- Integration tests against real audio/image fixtures

---

_Sprint 1 authored by: Claude (architect) + KR (approved)_
_Execute with: claude-opus-4-7 in ~/Wolfmark/augur/ (create it first)_
...
- apps/augur-cli/src/main.rs:30 — flip default
- crates/augur-classifier/src/classifier.rs — mark fasttext experimental
- CLAUDE.md — document the limitation
- crates/augur-classifier/examples/lid_label_probe.rs — commit as example
