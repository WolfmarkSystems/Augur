# VERIFY — Super Sprint: Production Hardening + New Capabilities + Full Integration
...
Create `crates/verify-classifier/src/arabic_dialect.rs`:
...
```
[VERIFY] Language: ar (Arabic) — confidence: HIGH
...
surveillance footage. They contain timestamped text that VERIFY
...
```bash
verify translate --input subtitles.srt --target en
```

Output:
```
[VERIFY] Input: subtitles.srt (SRT subtitle, 247 entries)
[VERIFY] Language: ar (Arabic) — dialect: Egyptian
[VERIFY] Translating 247 entries...
...
Integrating YARA with VERIFY creates a powerful combination:
...
Create `crates/verify-core/src/yara_scan.rs`:
...
impl YaraEngine {
    pub fn load(rules_path: &Path) -> Result<Self, VerifyError>;

    /// Scan text content (translated or original)
    pub fn scan_text(&self, text: &str) -> Result<Vec<YaraMatch>, VerifyError>;

    /// Scan a file
    pub fn scan_file(&self, path: &Path) -> Result<Vec<YaraMatch>, VerifyError>;
}
```
...
# Output:
[VERIFY] YARA scan: 3 matches found
...
fn yara_no_rules_path_returns_clear_error() {
    // Path to non-existent rules file → VerifyError, not panic
}
...
In production forensic use, VERIFY must handle corrupt files,
...
- Return `VerifyError::FileTooLarge { size, limit }`
...
where F: FnOnce() -> Result<T, VerifyError>
{
    // Use thread timeout or tokio timeout
    // Return VerifyError::ProcessTimeout if exceeded
}
```
...
where F: Fn() -> Result<T, VerifyError>
{
...
Create `apps/verify-cli/src/benchmark.rs`:
...
```bash
verify benchmark [--full] [--output results.json]
```

Output:
```
[VERIFY] Running benchmark suite...
...
```bash
verify benchmark --compare last_run.json
```
...
- [ ] `verify benchmark` command runs classification benchmarks
...
by running VERIFY as a Strata plugin against real evidence.

### Implementation

**Step 1 — Integration test with real evidence**

Add to `tests/` (gated on `VERIFY_RUN_INTEGRATION_TESTS=1`):
...
fn strata_plugin_processes_real_arabic_evidence() {
    ...
    // Run VerifyStrataPlugin::execute()
...
fn strata_plugin_metadata_complete() {
    let plugin = VerifyStrataPlugin::new("en");
...
## Adding VERIFY to Strata

1. Add to Strata's Cargo.toml:
   ```toml
   verify-plugin-sdk = { path = "../verify/crates/verify-plugin-sdk",
                         features = ["strata"] }
   ```
...
## What VERIFY adds to Strata

VERIFY surfaces foreign-language content as Communications
...
- [ ] `cargo build --features verify-plugin-sdk/strata` still passes
- [ ] 2 new tests pass
...
fn read_magic_bytes(path: &Path, n: usize) -> Result<Vec<u8>, VerifyError>;
...
```bash
verify translate --input suspicious_file --target en
```

Output:
```
[VERIFY] No extension detected — probing file content...
[VERIFY] Detected: PDF document (magic: %PDF)
[VERIFY] Processing as PDF...
```
...
VERIFY is production-ready but its documentation is scattered
...
VERIFY is an offline-first foreign language evidence processing tool.
...
### First-time setup
```bash
verify self-test          # check what's installed
verify self-test --full   # test full pipeline (downloads models)
```

### Classify a file
```bash
verify classify --text "مرحبا بالعالم" --target en
verify classify --file evidence.txt --target en
```
...
### Process a directory
```bash
verify batch --input /evidence/phone_dump --target en --output report.json
...
VERIFY downloads models on first use...
...
How to deploy VERIFY on a forensic workstation, including
...
- What VERIFY does (in plain language)
- Quick install
- Three example commands
- Link to full docs

**Step 5 — `verify --docs` command**

```bash
verify --docs           # opens USER_MANUAL.md in $PAGER or prints it
verify --docs geoip     # shows geoip-specific help
verify --docs languages # shows language support table
```
...
cargo build --features verify-plugin-sdk/strata 2>&1 | tail -3
```

Commit:
```bash
git add -A
git commit -m "feat: verify-super-sprint arabic dialect + SRT/VTT + YARA + hardening + docs"
```
...
- `verify self-test` output after all changes
...
_VERIFY Super Sprint authored by: Claude (architect) + KR (approved)_
...
_After this sprint, VERIFY is ready for field deployment._
