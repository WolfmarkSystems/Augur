# AUGUR Overnight Sprint — Production Polish + Real-World Testing
...
```bash
augur batch --input /evidence --target en --output report.csv
```
...
[AUGUR] Language detected: fa (Farsi) 
...
## PRIORITY 3 — AUGUR Self-Test Command

### Context

Examiners deploying AUGUR need a way to verify it's working
...
✓ STT: Whisper model cached (tiny preset)
  ⚠ STT inference: model not downloaded (run: augur self-test --full)
✓ Translation: NLLB model cached
  ⚠ Translation inference: requires Python + transformers
✓ OCR: Tesseract available
✓ Air-gap: AUGUR_AIRGAP_PATH not set (online mode)
...
- [ ] `augur self-test` command runs and outputs results
...
**Behavior:** AUGUR may classify Pashto text as Farsi (fa).
...
**Mitigation:** When Farsi is detected in evidence from a context
  ...
  linguist. AUGUR will surface an advisory notice when Farsi is
  detected with confidence below 0.90.

**Sprint reference:** AUGUR Sprint 5 (2026-04-25) confirmed this
...
**Behavior:** All language classifiers are less reliable on very
  short inputs. AUGUR surfaces a warning when input is < 10 words.
...
```bash
git add -A
git commit -m "feat: augur-overnight batch improvements + confidence tiers + self-test + Pashto advisory"
```

Report:
- Which priorities passed
- Test count before (61) and after
- Output of `augur self-test` command
...
_AUGUR Overnight Sprint authored by: Claude (architect) + KR (approved)_
