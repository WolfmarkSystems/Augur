...
## VERIFY Sprint 1 — 2026-04-24

**P1 Scaffold: PASSED**
  - Workspace created: yes — 6 crates + 1 app + `tests/fixtures/`, matches the tree in VERIFY_SPRINT_1.md exactly
...
**P2 Classifier: PASSED**
  ...
  - Classification tests: **8/8 pass** (`cargo test -p verify-classifier`)
    ...
    - `ModelManager::with_xdg_cache()` paths end with `.cache/verify/models` ✅
...
**P3 Whisper STT: PASSED (stub backend, real preprocessing)**
  ...
  - Integration tests: 2 present under `tests/whisper_integration.rs`, marked `#[ignore]`, gated on `VERIFY_RUN_INTEGRATION_TESTS=1`. `cargo test -- --include-ignored` without the env var prints a skipping message and does nothing network — no false egress from a naïve override.

**P4 CLI: PASSED**
  - Binary: `apps/verify-cli/src/main.rs`, bin name `verify`, `cargo build --release` produces a 2.6 MB binary in 14 s.
  - Subcommands (all working):
    - `verify classify --text <s> --target <iso>` — real classification via fastText or whichlang
    - `verify transcribe --input <path> --preset [fast|balanced|accurate]` — surfaces STT stub cleanly with `[STT stub — Sprint 2]`
    - `verify translate --input <path> --target <iso> --preset [...]` — full pipeline: runs STT (stub) → classifies transcript → `translate_stub` returns the sentinel. Output format:
      ```
      [VERIFY] Language detected: ar (confidence: 0.97)
      [VERIFY] Transcript: [STT stub — Sprint 2]
  ...
  - `verify --version` / `-V` → **exact** `VERIFY 0.1.0 — Wolfmark Systems` (disabled clap's default `{bin_name} {version}` shape and intercepted the flag ourselves; the string lives in a `const VERSION_STRING` so it's greppable and tracks `CARGO_PKG_VERSION`).
  - `verify --help` shows *"All processing is local. No evidence leaves your machine."* as the offline-invariant banner at the top of `long_about`.
  - `verify` with no subcommand prints `[VERIFY] no subcommand given. Run \`verify --help\` for usage.` and exits 2 (matches clap's usage-error convention).
...
- `cargo build --release -p verify-cli`: clean, 2.6 MB binary produced
...
crates/verify-classifier/src/classifier.rs:const LID_MODEL_URL: &str = "https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.ftz";
crates/verify-stt/src/whisper.rs:pub const WHISPER_MODEL_URL_TINY:     &str = "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-tiny.bin";
crates/verify-stt/src/whisper.rs:pub const WHISPER_MODEL_URL_BASE:     &str = "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.bin";
crates/verify-stt/src/whisper.rs:pub const WHISPER_MODEL_URL_LARGE_V3: &str = "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-large-v3.bin";
```
...
- `945268e` — `chore: scaffold VERIFY workspace — 6 crates, CLI app, CLAUDE.md`
- `82e5c5a` — `feat: verify-classifier — fastText LID + whichlang fallback, ModelManager with one-time download, 8 unit tests`
- `a415b33` — `chore: add *.ftz to .gitignore — fastText model weights`
- `5a32b73` — `feat: verify-stt — Whisper STT engine, 3 presets, audio preprocessing (ffmpeg+hound), 8 unit + 2 ignored integration tests — whisper-rs deferred to Sprint 2 (cmake FFI gap)`
- `79de0b6` — `feat: verify-cli — classify + transcribe + translate stub subcommands, --classifier-backend flag, exact VERIFY 0.1.0 version string`
...
- **NLLB-200 translation** — replaces `verify-translate::TRANSLATION_STUB`. Add `NLLB_MODEL_URL_*` constants alongside the existing egress constants so the four-URL audit grep extends cleanly.
- **Tesseract OCR** — wire `leptess` + language packs; replaces `verify-ocr::ocr_image_stub`.
- **Video pipeline** — ffmpeg audio extract → STT → classifier → translate.
- **`verify-plugin-sdk`** — wire to the real `strata-plugin-sdk::StrataPlugin` trait so VERIFY surfaces inside Strata as an artifact emitter.
- **Integration tests vs real fixtures** — flip the two `#[ignore]` tests in `verify-stt/tests/whisper_integration.rs` to actual assertions once a real STT backend is wired.
