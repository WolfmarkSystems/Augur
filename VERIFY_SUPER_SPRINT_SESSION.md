# VERIFY Super Sprint — 2026-04-26 — Session log
...
- New `verify_classifier::arabic_dialect` module:
...
- New `verify_core::subtitle` module — `SubtitleEntry`,
...
- New `verify_core::yara_scan` module: `YaraEngine`,
...
- New `VerifyError::Yara` + `VerifyError::YaraNotInstalled`
...
- New `verify_core::resilience` module: `PipelineLimits`
...
- New `VerifyError::FileTooLarge { size_bytes, limit_bytes }`,
  `VerifyError::CorruptFile { path, reason }`,
  `VerifyError::ProcessTimeout { seconds }` variants.
...
- New `apps/verify-cli/src/benchmark.rs` module with
  `BenchmarkSuite`, `BenchmarkResult`, `run_suite`,
  `render_text`, `render_regression_report`. New CLI command
  `verify benchmark` with `--full` (translation pass) and
...
### P8 User docs + README + verify --docs
...
- New `verify docs [topic]` subcommand. Topics: `manual`
...
- **Default-build test count: 166 passing** (up from
  ...
  `VERIFY_RUN_INTEGRATION_TESTS=1` (now 5 with the new
  Strata live-evidence test).
- **`--features verify-plugin-sdk/strata` build**: 173 unit
...
## `verify self-test` output (post-sprint)

```
[VERIFY] Running self-test...

[VERIFY] ✓ [PASS] Classification: Arabic text → ar
[VERIFY] ✓ [PASS] Classification: English → not foreign
[VERIFY] ✓ [PASS] Classification: empty input → handled
[VERIFY] ✓ [PASS] Pashto/Farsi script disambiguation (confidence 0.95)
[VERIFY] ⚠ [WARN] Audio preprocessing: ffmpeg
[VERIFY] ⚠ [WARN] OCR: tesseract
[VERIFY] ⚠ [WARN] PDF rasterize: pdftoppm
[VERIFY] ✓ [PASS] STT: Whisper tiny safetensors cached
[VERIFY] ✓ [PASS] Translation: NLLB-200 cached
[VERIFY] ✓ [PASS] Air-gap mode (online-on-first-run)
[VERIFY] ⚠ [SKIP] HF token not configured
[VERIFY] ⚠ [SKIP] GeoIP: GeoLite2 database not configured
[VERIFY] ✓ [PASS] Offline invariant audit

[VERIFY] Self-test PASSED (8 passed, 0 failed, 2 skipped, 3 warnings)
...
- **`verify docs`** prints through `println_verify` rather
  than raw stdout, keeping every CLI line routed through
  the one auditable function. Strips with `sed 's/^\[VERIFY\] //'`
  for clean reading.
