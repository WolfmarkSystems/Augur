...
In `apps/verify-cli/src/main.rs` line 30:
...
In `crates/verify-classifier/src/classifier.rs`, update the
...
`crates/verify-classifier/examples/lid_label_probe.rs` was
written but not committed in Sprint 1. Commit it now as a
non-default `[[example]]` entry in `verify-classifier/Cargo.toml`:
...
In `crates/verify-stt/src/whisper.rs`:
...
Add `--temperature` and `--max-retries` flags to `verify transcribe`
and `verify translate`:

```bash
verify transcribe --input noisy.mp3 --max-retries 5
```
...
VERIFY should handle it natively.
...
In `crates/verify-ocr/src/lib.rs`:
...
pub fn extract_pdf_text(
    ...
    let text = pdf_extract::extract_text(pdf_path)
        .map_err(|e| VerifyError::Ocr(e.to_string()))?;
...
verify batch --input /evidence --types audio,video,image,pdf --target en
```
...
time python3 crates/verify-translate/scripts/worker_script.py \
    --text "$(cat tests/fixtures/arabic_100_words.txt)" \
    --src ara_Arab --tgt eng_Latn \
    --backend transformers

# Time ctranslate2 backend  
time python3 crates/verify-translate/scripts/worker_script_ct2.py \
    --text "$(cat tests/fixtures/arabic_100_words.txt)" \
    --src ara_Arab --tgt eng_Latn
```
...
```
fix: verify-sprint-4-P1 whichlang default + fasttext experimental label
feat: verify-sprint-4-P2 Whisper temperature fallback + hallucination guard
feat: verify-sprint-4-P3 PDF extraction — text layer + OCR fallback
docs: verify-sprint-4-P4 ctranslate2 benchmark results + recommendation
```
...
_After Sprint 4, VERIFY is production-ready for v1.0 release._
