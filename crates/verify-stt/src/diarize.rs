...
//!
//! Sprint 5 P2. Implementation strategy mirrors `verify-translate`:
...
//! the gated `pyannote/speaker-diarization-3.1` model. VERIFY
//! manages that token via [`HfTokenManager`] — it is read from
//! `~/.cache/verify/hf_token` (chmod 600) and is the first VERIFY
...
/// Manages the on-disk Hugging Face token VERIFY uses to download
/// gated pyannote weights. The token lives at
/// `~/.cache/verify/hf_token` and is treated as a secret — we
...
impl HfTokenManager {
    /// Default path: `~/.cache/verify/hf_token`.
    pub fn with_xdg_cache() -> Result<Self, VerifyError> {
        ...
        Ok(Self {
            token_path: PathBuf::from(home).join(".cache/verify/hf_token"),
        })
    }
    ...
    /// read it.
    pub fn save(&self, token: &str) -> Result<(), VerifyError> {
        ...
        info!("verify-stt: HF token written to {:?}", self.token_path);
        Ok(())
    }
}
...
impl DiarizationEngine {
    ...
    /// HF cache at `~/.cache/verify/models/pyannote/`.
    pub fn with_xdg_cache() -> Result<Self, VerifyError> {
        let home = std::env::var("HOME").map_err(|_| {
            VerifyError::Stt("HOME not set; pass cache dir explicitly".to_string())
        })?;
        Ok(Self {
            python_cmd: "python3".into(),
            model: DEFAULT_PYANNOTE_MODEL.into(),
            token_manager: HfTokenManager::with_xdg_cache()?,
            hf_cache: Some(PathBuf::from(home).join(".cache/verify/models/pyannote")),
        })
    }
    ...
    pub fn diarize(&self, audio_path: &Path) -> Result<Vec<DiarizationSegment>, VerifyError> {
        ...
        warn!(
            "verify-stt: invoking pyannote diarization ({}) on {:?} — \
        ...
        let request = DiarizeRequest {
            audio_path: audio_path
                .to_str()
                .ok_or_else(|| VerifyError::Stt(format!("non-UTF8 path: {audio_path:?}")))?,
            hf_token: &token,
            model: &self.model,
        };
        let req_json = serde_json::to_string(&request).map_err(|e| {
            VerifyError::Stt(format!("diarize request serialise: {e}"))
        })?;
        ...
        if let Some(stdin) = child.stdin.as_mut() {
            stdin
                ...
                .map_err(|e| VerifyError::Stt(format!("write stdin: {e}")))?;
        } else {
        ...
        let output = child
            .wait_with_output()
            .map_err(|e| VerifyError::Stt(format!("child wait: {e}")))?;

        let stdout = String::from_utf8_lossy(&output.stdout).to_string();
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        debug!(
            "verify-stt: pyannote exit={:?} stderr_bytes={} stdout_bytes={}",
            output.status.code(),
            stderr.len(),
            stdout.len()
        );
        ...
        let segments = resp
            .segments
            .ok_or_else(|| VerifyError::Stt("pyannote returned no segments".into()))?
            .into_iter()
...
mod tests {
    ...
    fn hf_token_manager_returns_clear_error_when_missing() {
        ...
        match bogus.load() {
            Err(VerifyError::Stt(msg)) => {
                assert!(
                    msg.contains("verify setup --hf-token"),
    ...
    fn save_rejects_empty_token() {
        let tmp = std::env::temp_dir().join("verify-hf-token-empty-test");
