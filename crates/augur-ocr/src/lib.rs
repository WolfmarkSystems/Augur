...
impl OcrEngine {
    ...
    pub fn extract_text(&self, image_path: &Path) -> Result<OcrResult, AugurError> {
        ...
        if !which_binary(&self.tesseract_cmd) {
            return Err(AugurError::Ocr(format!(
        ...
        debug!("augur-ocr: extracted {} chars", trimmed.len());
        ...
        info!(
            "augur-ocr: {} words extracted from {:?}",
            words.len(),
            image_path
        );
...
pub fn iso_to_tesseract(iso: &str) -> Result<&'static str, AugurError> {
    Ok(match iso {
        "ar" => "ara",
        "en" => "eng",
        "fa" => "fas",
...
pub fn extract_pdf_text(
    ...
    debug!("augur-ocr: extracting text layer from {pdf_path:?}");
    let text_layer = match pdf_extract::extract_text(pdf_path) {
        Ok(t) => t,
        Err(e) => {
            warn!("augur-ocr: pdf-extract failed on {pdf_path:?}: {e}");
            String::new()
        }
    };
    if !text_layer.trim().is_empty() {
        info!(
            "augur-ocr: PDF text layer extracted: {} chars",
            text_layer.len()
        );
        return Ok(text_layer);
    }
    debug!("augur-ocr: no text layer in {pdf_path:?} — falling back to rasterize+OCR");
...
fn extract_pdf_via_ocr(
    ...
    warn!(
        "augur-ocr: rasterizing {pdf_path:?} via pdftoppm → {prefix:?}-N.png \
    ...
    let status = Command::new("pdftoppm")
        ...
        .arg(&prefix)
        .status()
        .map_err(|e| AugurError::Ocr(format!("failed to launch pdftoppm: {e}")))?;
    if !status.success() {
        return Err(AugurError::Ocr(format!(
    ...
    let prefix_str = prefix
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| AugurError::Ocr("scratch prefix not UTF-8".to_string()))?
        .to_string();
    ...
    for page in &pages {
        match engine.extract_text(page) {
            ...
            Err(e) => {
                warn!("augur-ocr: OCR failed on PDF page {page:?}: {e}");
            }
        }
...
pub fn ocr_image_stub(path: &Path, lang_hint: Option<&str>) -> Result<String, AugurError> {
...
mod tests {
    ...
    fn pdf_extraction_returns_error_for_missing_file() {
        ...
        let scratch = std::env::temp_dir().join("augur-pdf-test-missing");
    ...
    fn extract_pdf_via_ocr_rejects_unmapped_lang() {
        ...
        let scratch = std::env::temp_dir().join("augur-pdf-test-bad-lang");
