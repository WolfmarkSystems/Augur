...
//!
//! Reproduce:
//!   cargo run -p augur-classifier --features fasttext-probe \
...
fn main() {
    ...
    let model = PathBuf::from(home).join(".cache/augur/models/lid.176.ftz");
    if !model.exists() {
        eprintln!("lid.176.ftz not found at {model:?}; run `augur classify` once to seed the cache.");
        std::process::exit(2);
    }
