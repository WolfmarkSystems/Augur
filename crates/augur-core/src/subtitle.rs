...
//! transcripts). They carry timestamped text that AUGUR can
...
pub fn parse_srt_timestamp(s: &str) -> Result<u64, AugurError> {
    parse_timestamp(s, ',')
}

/// Parse `HH:MM:SS.mmm` (WebVTT) into milliseconds.
pub fn parse_vtt_timestamp(s: &str) -> Result<u64, AugurError> {
    parse_timestamp(s, '.')
}

fn parse_timestamp(s: &str, frac_sep: char) -> Result<u64, AugurError> {
    let s = s.trim();
    let (hms, frac) = s
        .split_once(frac_sep)
        .ok_or_else(|| AugurError::InvalidInput(format!("missing '{frac_sep}' in {s:?}")))?;
    ...
    let h: u64 = parts[0]
        .parse()
        .map_err(|e| AugurError::InvalidInput(format!("hours parse {parts:?}: {e}")))?;
    let m: u64 = parts[1]
        .parse()
        .map_err(|e| AugurError::InvalidInput(format!("minutes parse {parts:?}: {e}")))?;
    let s_int: u64 = parts[2]
        .parse()
        .map_err(|e| AugurError::InvalidInput(format!("seconds parse {parts:?}: {e}")))?;
    let ms: u64 = frac
        .parse()
        .map_err(|e| AugurError::InvalidInput(format!("ms parse {frac:?}: {e}")))?;
...
pub fn parse_srt(content: &str) -> Result<Vec<SubtitleEntry>, AugurError> {
    ...
    for block in normalized.split("\n\n") {
        ...
        let (start, end) = ts_line
            .split_once("-->")
            .ok_or_else(|| AugurError::InvalidInput("split '-->'".to_string()))?;
...
/// since AUGUR only cares about the timestamped text.
pub fn parse_vtt(content: &str) -> Result<Vec<SubtitleEntry>, AugurError> {
    let body = strip_bom(content);
    let normalized = body.replace("\r\n", "\n");
    let mut out: Vec<SubtitleEntry> = Vec::new();
    for block in normalized.split("\n\n") {
        ...
        let (start, end_with_settings) = ts_line
            .split_once("-->")
            .ok_or_else(|| AugurError::InvalidInput("vtt split '-->'".to_string()))?;
