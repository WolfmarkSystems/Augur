...
//! file (created by `augur config init`); a missing config still
...
impl ReportConfig {
    ...
    /// `augur config init` contains the same fields so an
    ...
    pub fn load(path: &Path) -> Result<Self, AugurError> {
        let body = std::fs::read_to_string(path)?;
        Self::from_toml_str(&body)
    }

    pub fn from_toml_str(s: &str) -> Result<Self, AugurError> {
        let parsed: TomlConfig = toml::from_str(s).map_err(|e| {
            AugurError::InvalidInput(format!("config TOML parse: {e}"))
        })?;
    ...
    /// Serialize back to TOML for `augur config init` /
    /// `augur config show`. Always emits the full schema with
    /// defaults filled in so the user can see every knob.
    pub fn to_toml_string(&self) -> Result<String, AugurError> {
        ...
        toml::to_string_pretty(&wrapped).map_err(|e| {
            AugurError::InvalidInput(format!("config TOML serialize: {e}"))
        })
    }
...
<head>
<meta charset="utf-8">
<title>AUGUR — Foreign Language Analysis</title>
<style>
...
pub fn render_batch_html(report: &BatchResult, config: &ReportConfig) -> String {
    ...
    let title = config
        .report_title
        .as_deref()
        .unwrap_or("AUGUR Foreign Language Analysis Report");
...
mod tests {
    ...
    fn json_metadata_includes_agency_when_configured() {
        ...
        assert!(serialized.contains("augur_version"));
    }
