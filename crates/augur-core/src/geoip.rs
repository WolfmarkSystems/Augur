...
//! # Offline invariant
//!
//! MaxMind's license terms bar AUGUR from auto-downloading the
//! GeoLite2 database. The examiner MUST place
//! `GeoLite2-City.mmdb` somewhere AUGUR can find it:
//! - explicit path via `AUGUR_GEOIP_PATH`
//! - or the XDG default `~/.cache/augur/GeoLite2-City.mmdb`
//!
//! When neither resolves, [`GeoIpEngine::with_xdg_cache`] returns
//! [`AugurError::GeoIpNotConfigured`] with the download
...
/// [`AugurError::GeoIpNotConfigured`] and printed by the CLI's
/// `augur geoip --setup` subcommand.
pub const GEOIP_DB_INSTRUCTIONS: &str =
    "Download GeoLite2-City.mmdb from \
     ...
     AUGUR_GEOIP_PATH=/path/to/GeoLite2-City.mmdb or place it at \
     ~/.cache/augur/GeoLite2-City.mmdb.";
...
impl GeoIpEngine {
    ...
    /// XDG-style construction: prefer `$AUGUR_GEOIP_PATH`,
    /// otherwise look for `~/.cache/augur/GeoLite2-City.mmdb`.
    /// Returns [`AugurError::GeoIpNotConfigured`] (with the
    ...
    pub fn with_xdg_cache() -> Result<Self, AugurError> {
        ...
        Err(AugurError::GeoIpNotConfigured(
            GEOIP_DB_INSTRUCTIONS.to_string(),
        ))
    }

    /// Look up a single IP. Returns `Err(AugurError::GeoIp)` for
    ...
    pub fn lookup(&self, ip: &str) -> Result<GeoIpResult, AugurError> {
        let parsed: IpAddr = ip
            .parse()
            .map_err(|e| AugurError::GeoIp(format!("not a valid IP {ip:?}: {e}")))?;
        ...
        let lookup_result = self
            .reader
            .lookup(parsed)
            .map_err(|e| AugurError::GeoIp(format!("MaxMind lookup({ip}): {e}")))?;
        let decoded: Option<maxminddb::geoip2::City> = lookup_result
            .decode()
            .map_err(|e| AugurError::GeoIp(format!("MaxMind decode({ip}): {e}")))?;
...
pub fn configured_db_path() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("AUGUR_GEOIP_PATH") {
...
/// Status string for `augur self-test`. `Ok(path)` on success,
...
mod tests {
    ...
    fn geoip_not_configured_returns_clear_error() {
        ...
        let prev_geoip = std::env::var("AUGUR_GEOIP_PATH").ok();
        // SAFETY: serialized via env_lock(); restored at end.
        unsafe {
            std::env::set_var("HOME", "/tmp/augur-geoip-no-such-dir-xyz");
            std::env::remove_var("AUGUR_GEOIP_PATH");
        }
        let r = GeoIpEngine::with_xdg_cache();
        match &r {
            Err(AugurError::GeoIpNotConfigured(msg)) => {
                assert!(
                    msg.contains("AUGUR_GEOIP_PATH"),
        ...
        unsafe {
            ...
            match prev_geoip {
                Some(v) => std::env::set_var("AUGUR_GEOIP_PATH", v),
                None => std::env::remove_var("AUGUR_GEOIP_PATH"),
            }
        }
    }
    ...
    fn load_missing_db_returns_not_configured_error() {
        ...
        match GeoIpEngine::load(bogus) {
            Err(AugurError::GeoIpNotConfigured(_)) => {}
            other => panic!("expected GeoIpNotConfigured, got {other:?}"),
        }
    }
