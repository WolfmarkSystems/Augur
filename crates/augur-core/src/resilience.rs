...
/// AUGUR Sprint super-sprint spec (500 MB audio/video, 10 MB
...
pub fn check_file_size(path: &Path, limits: &PipelineLimits) -> Result<(), AugurError> {
    let meta = std::fs::metadata(path).map_err(|e| {
        AugurError::CorruptFile {
    ...
    let size = meta.len();
    if size == 0 {
        return Err(AugurError::CorruptFile {
            path: path.to_string_lossy().into_owned(),
            reason: "empty file (0 bytes)".to_string(),
        });
    }
    if size > limits.max_file_size_bytes {
        return Err(AugurError::FileTooLarge {
            size_bytes: size,
...
pub fn check_text_size(text: &str, limits: &PipelineLimits) -> Result<(), AugurError> {
    let len = text.len();
...
pub fn with_retry<F, T>(max_attempts: u32, mut f: F) -> Result<T, AugurError>
where
    F: FnMut() -> Result<T, AugurError>,
{
    assert!(max_attempts >= 1, "with_retry requires max_attempts >= 1");
    let mut last_err: Option<AugurError> = None;
...
mod tests {
    ...
    fn empty_file_returns_corrupt_not_panic() {
        ...
        match check_file_size(&path, &PipelineLimits::default()) {
            Err(AugurError::CorruptFile { reason, .. }) => {
    ...
    fn retry_succeeds_on_third_attempt() {
        ...
        let result: Result<i32, AugurError> = with_retry(5, || {
            attempts.set(attempts.get() + 1);
            if attempts.get() < 3 {
                Err(AugurError::Stt("transient".to_string()))
            } else {
                Ok(42)
            }
        });
    ...
    fn retry_returns_last_error_on_exhaustion() {
        let result: Result<i32, AugurError> = with_retry(2, || {
            Err(AugurError::Stt("perma-fail".to_string()))
        });
        match result {
            Err(AugurError::Stt(msg)) => assert_eq!(msg, "perma-fail"),
