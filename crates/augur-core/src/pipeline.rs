...
//!
//! augur-core stays lean: it owns the unified [`AugurError`] and
...
//! eventually in the Strata plugin adapter), so that augur-core
//! can stay free of ML / audio / image dependencies.
//!
//! This split avoids a dependency cycle: each sub-engine depends on
//! augur-core for the error type, so augur-core cannot in turn
//! depend on the sub-engines.
//!
//! [`AugurError`]: crate::error::AugurError
...
pub enum PipelineInput {
    ...
    /// Parser is in `augur_core::subtitle`; the CLI translates
...
/// who points AUGUR at `interview.mp4` does not need to remember
...
/// Plain timestamped segment. Mirrors `augur_stt::SttSegment` but
...
//
// Sprint 3: AUGUR can process an entire directory in one
...
/// CSV header row in the order spec'd by AUGUR_SPRINT_6.md P1a.
