"""NLLB-200 offline translation worker invoked by augur-translate.

AUGUR's offline-first invariant: model weights are downloaded once
...
def main() -> int:
    ...
    # Force the HF cache under AUGUR_HF_CACHE if set so all AUGUR
    # weights land under one auditable directory.
    cache = os.environ.get("AUGUR_HF_CACHE")
    if cache:
