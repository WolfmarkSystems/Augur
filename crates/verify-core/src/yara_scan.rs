...
//! [`VerifyError::YaraNotInstalled`] error with the install
//! hint.
//!
//! # Why subprocess
//!
//! The `yara` Rust crate links against `libyara` system
//! headers; that's a heavier system dep than VERIFY needs for
//! pattern matching. The `yara` CLI is universally available
//! on forensic workstations (`brew install yara`,
//! `apt install yara`) and produces structured output VERIFY
//! parses cheaply.
//!
//! # Output format
//!
...
impl YaraEngine {
    ...
    pub fn load(rules_path: &Path) -> Result<Self, VerifyError> {
        if !rules_path.exists() {
            return Err(VerifyError::Yara(format!(
    ...
    fn scan_file_internal(
        ...
        target: &Path,
        scanned_label: &str,
    ) -> Result<Vec<YaraMatch>, VerifyError> {
        if !self.is_available() {
            return Err(VerifyError::YaraNotInstalled(format!(
                "`{}` not found on PATH. Install YARA: \
                 `brew install yara` (macOS) or `apt install yara` (Linux). \
                 VERIFY's --yara-rules feature is opt-in.",
                self.yara_cmd
            )));
        }
        let output = Command::new(&self.yara_cmd)
            ...
            .stdin(Stdio::null())
            .output()
            .map_err(|e| VerifyError::Yara(format!("spawn yara: {e}")))?;
...
mod tests {
    ...
    fn yara_scan_returns_not_installed_when_yara_missing() {
        ...
        match engine.scan_text("anything") {
            Err(VerifyError::YaraNotInstalled(msg)) => {
