//! Real `strata_plugin_sdk::StrataPlugin` implementation for AUGUR.
//!
...
//! # Plugin behavior
//!
//! Given a `PluginContext` rooted at materialized evidence, AUGUR:
...
use augur_classifier::LanguageClassifier;
use augur_core::pipeline::{detect_input_kind, PipelineInput};
...
use augur_translate::{TranslationEngine, TranslationResult};

const ADVISORY_TITLE_PREFIX: &str = "[MT — review by a certified human translator] ";
const AUGUR_SUBCATEGORY: &str = "Foreign Language Translation";
...
impl StrataPlugin for crate::AugurStrataPlugin {
    fn name(&self) -> &str {
        "AUGUR"
    }
    ...
    fn supported_inputs(&self) -> Vec<String> {
        // AUGUR processes audio / video / image artefacts on the
    ...
    fn run(&self, context: PluginContext) -> PluginResult {
        ...
        for r in &records {
            ...
            artifacts.push(Artifact {
                category: AUGUR_SUBCATEGORY.into(),
                timestamp: None,
    ...
    fn execute(&self, context: PluginContext) -> Result<PluginOutput, PluginError> {
        ...
        Ok(PluginOutput {
            ...
            summary: PluginSummary {
                ...
                headline: format!("AUGUR: {total} foreign-language artifact(s) translated"),
            },
            warnings: vec![],
        })
    }
...
pub fn record_from_translation(
    ...
    ArtifactRecord {
        ...
        subcategory: AUGUR_SUBCATEGORY.into(),
        timestamp: None,
        title: format!("{ADVISORY_TITLE_PREFIX}AUGUR Translation: {file_name}"),
...
fn assert_advisory_invariant(r: &ArtifactRecord) -> Result<(), String> {
    ...
    let is_mt = raw
        ...
        .unwrap_or(false);
    if !is_mt {
        return Err("is_machine_translation must be true on every AUGUR artifact".into());
    }
    Ok(())
}
...
pub fn walk_and_translate(
    ...
    for file in &files {
        match process_one_file(file, target_language, &classifier, &translation_engine) {
            ...
            Err(e) => log::warn!("augur-strata-plugin: skipping {file:?}: {e}"),
        }
    }
    Ok(records)
}
...
fn build_classifier() -> LanguageClassifier {
    use augur_classifier::ModelManager;
    ...
    log::warn!(
        "augur-strata-plugin: fasttext unavailable; falling back to whichlang \
...
fn process_one_file(
    ...
    let resolved = match &kind {
        ...
        // translations should use `augur translate --output-srt`
...
struct ResolvedSource {
    ...
    segments: Option<Vec<augur_stt::SttSegment>>,
}
...
fn resolve_video(file: &Path) -> Result<ResolvedSource, String> {
    let scratch = std::env::temp_dir().join("augur").join("strata-video-scratch");
...
fn run_stt(file: &Path) -> Result<SttResult, augur_core::AugurError> {
...
mod tests {
    use super::*;
    use augur_translate::{DEFAULT_NLLB_MODEL, MACHINE_TRANSLATION_NOTICE};
    ...
    fn assert_advisory_invariant_rejects_missing_prefix() {
        ...
        r.title = "AUGUR Translation: clip.mp3".into();
    ...
    /// test. Gated on `AUGUR_RUN_INTEGRATION_TESTS=1` because
    ...
    #[test]
    #[ignore = "AUGUR_RUN_INTEGRATION_TESTS=1 — drives walk_and_translate against a temp dir"]
    fn strata_plugin_processes_real_arabic_evidence() {
        if std::env::var("AUGUR_RUN_INTEGRATION_TESTS").ok().as_deref() != Some("1") {
            eprintln!("AUGUR_RUN_INTEGRATION_TESTS != 1 — skipping integration body");
            return;
        }
        let work = std::env::temp_dir().join(format!(
            "augur-strata-live-{}-{}",
    ...
    fn strata_plugin_metadata_complete() {
        let p = crate::AugurStrataPlugin::new();
        assert!(!<crate::AugurStrataPlugin as StrataPlugin>::name(&p).is_empty());
    ...
    fn plugin_metadata_via_real_trait() {
        let p = crate::AugurStrataPlugin::new();
        assert_eq!(<crate::AugurStrataPlugin as StrataPlugin>::name(&p), "AUGUR");
        assert_eq!(p.required_tier(), PluginTier::Professional);
        assert!(matches!(p.plugin_type(), PluginType::Analyzer));
        assert!(p
            .capabilities()
