# VERIFY Sprint 8 — Strata Plugin Wiring + Multi-Language Evidence + Video Diarization
...
`verify-plugin-sdk/strata` with a placeholder implementation.
...
implement the real trait, and wire VERIFY as a first-class Strata plugin.
...
In `crates/verify-plugin-sdk/src/strata_impl.rs`:
...
impl StrataPlugin for VerifyStrataPlugin {
...
Document in CLAUDE.md how to add VERIFY to Strata's PLUGIN_NAMES.
The actual registration happens in Strata's codebase — VERIFY
...
English emails, and Chinese WeChat exports. VERIFY's batch processor
...
verify batch --input /evidence --targets ar:en,ru:en

# Auto-detect all foreign languages and translate to English
verify batch --input /evidence --target en --all-foreign
```
...
In `crates/verify-core/src/pipeline.rs`, when processing video:
...
Output:
```
[VERIFY] Input: interview.mp4 (video)
[VERIFY] Extracting audio...
[VERIFY] Running STT (Whisper Fast)...
[VERIFY] Language detected: ar (Arabic)
[VERIFY] Running speaker diarization...
[VERIFY] Speakers detected: 2 (SPEAKER_00, SPEAKER_01)
...
```bash
git add -A
git commit -m "feat: verify-sprint-8 Strata plugin wiring + multi-language batch + video diarization"
```

Report:
- Which priorities passed
- Test count before (105) and after
- Whether `cargo build --features verify-plugin-sdk/strata` succeeds
- Any deviations from spec

---

_VERIFY Sprint 8 authored by: Claude (architect) + KR (approved)_
