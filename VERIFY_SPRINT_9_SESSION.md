# VERIFY Sprint 9 — 2026-04-26 — Session log
...
- New `verify_classifier::script` module:
...
- CLI: `print_classification` now prints the note as a second
  `⚠` line under the existing advisory. `verify self-test`
...
- `rayon = "1.12"` added as a direct dep of `verify-cli`. Pool
...
- Default thread count: `0` resolves to
  ...
  `verify batch --threads <N>` (also wired into
  `verify package`); `--threads 1` forces sequential behaviour
...
- New `apps/verify-cli/src/package.rs` module — `Manifest`,
...
- New CLI subcommand `verify package --input <dir>
  [--output <zip>] [--target en] [--config <toml>]
  [--preset balanced] [--ocr-lang en] [--include-originals]`.
  Default output: `verify-package-YYYYMMDD.zip` in the cwd.
...
- **`--features verify-plugin-sdk/strata` build**: 137 unit
...
- Spec listed ژ in both Pashto-specific AND Farsi-specific
  ...
  glyphs without it.
- `verify package` runs its own internal parallel batch pass
