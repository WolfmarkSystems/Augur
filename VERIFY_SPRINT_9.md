# VERIFY Sprint 9 — Pashto Disambiguation + Batch Parallel + Evidence Integration
...
Create `crates/verify-classifier/src/script.rs`:
...
```
[VERIFY] Language detected: fa (Farsi/Persian)
...
Add a disambiguation check to `verify self-test`:
...
In `crates/verify-core/src/pipeline.rs`, replace the sequential
...
pub fn run_batch_parallel(
    ...
    let pool = rayon::ThreadPoolBuilder::new()
        ...
        .build()
        .map_err(|e| VerifyError::Batch(e.to_string()))?;
...
verify batch --input /evidence --target en --threads auto  # default
```
...
After running VERIFY on an evidence directory, examiners need
...
VERIFY translations, the batch report, and a manifest.
...
```bash
verify package --input /evidence --output case-001-verify.zip
```
...
│   ├── recording_001.mp3
│   └── doc_002.pdf
└── CHAIN_OF_CUSTODY.txt   ← who ran VERIFY, when, on what system
```
...
```
VERIFY Evidence Package — Chain of Custody
==========================================
...
All translations in this package were produced by VERIFY,
...
- [ ] `verify package` command creates zip
...
```bash
git add -A
git commit -m "feat: verify-sprint-9 Pashto disambiguation + parallel batch + evidence package"
```
...
_VERIFY Sprint 9 authored by: Claude (architect) + KR (approved)_
