...
use verify_core::pipeline::{BatchFileResult, BatchResult};
use verify_core::report::{render_batch_html, ReportConfig};
use verify_core::VerifyError;
use verify_translate::MACHINE_TRANSLATION_NOTICE;
...
pub fn render_chain_of_custody(
    ...
    out.push_str("VERIFY Evidence Package — Chain of Custody\n");
    out.push_str("==========================================\n");
    out.push_str(&format!(
        "Package created: {}\n",
        report.generated_at,
    ...
    out.push_str("--------------------------\n");
    out.push_str("All translations in this package were produced by VERIFY,\n");
...
pub fn sha256_of_path(path: &Path) -> Result<String, VerifyError> {
    use std::io::Read;
...
pub fn write_package(
    ...
    let manifest_json = serde_json::to_string_pretty(&manifest).map_err(|e| {
        VerifyError::Translate(format!("MANIFEST.json serialize: {e}"))
    })?;
    ...
    let report_json = serde_json::to_string_pretty(report).map_err(|e| {
        VerifyError::Translate(format!("REPORT.json serialize: {e}"))
    })?;
    ...
    zip.finish().map_err(|e| {
        VerifyError::Translate(format!("zip finalise: {e}"))
    })?;
    Ok(manifest)
}
...
fn write_zip_entry(
    ...
    zip.start_file(name, opts).map_err(|e| {
        VerifyError::Translate(format!("zip start_file({name}): {e}"))
    })?;
    zip.write_all(body)
        .map_err(|e| VerifyError::Translate(format!("zip write({name}): {e}")))?;
    Ok(())
}

fn build_translation_text(r: &BatchFileResult) -> String {
    let mut body = String::with_capacity(512);
    body.push_str("# VERIFY translation\n");
...
mod tests {
    use super::*;
    use verify_core::pipeline::{BatchFileResult, BatchResult, BatchSummary};
    ...
    fn package_manifest_sha256_correct() {
        ...
        let content = b"verify-pkg-test-content";
