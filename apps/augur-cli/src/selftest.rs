//! `augur self-test` — pre-deployment readiness check.
//!
//! Sprint 6 P3. Examiners deploying AUGUR need a one-shot way to
...
//!   `~/.cache/augur/models/` directly.

use std::path::{Path, PathBuf};
use augur_classifier::{ClassificationResult, ConfidenceTier, LanguageClassifier};
...
pub fn run_pashto_disambiguation_check() -> SelfTestCheck {
    ...
    let analysis = augur_classifier::pashto_farsi_score(probe);
    use augur_classifier::ScriptRecommendation;
...
pub fn run_cached_artifact_check(name: &str, path: &Path) -> SelfTestCheck {
    if path.exists() {
        ...
        SelfTestCheck {
            ...
            message: format!(
                "not cached at {path:?} (run `augur self-test --full` to download \
...
pub fn run_airgap_check() -> SelfTestCheck {
    match std::env::var("AUGUR_AIRGAP_PATH") {
        Ok(p) if !p.is_empty() => SelfTestCheck {
            name: "Air-gap mode".into(),
            status: CheckStatus::Pass,
            message: format!("AUGUR_AIRGAP_PATH set to {p}"),
        },
        _ => SelfTestCheck {
            name: "Air-gap mode".into(),
            status: CheckStatus::Pass,
            message: "AUGUR_AIRGAP_PATH not set (online-on-first-run mode)".into(),
        },
    }
}
...
pub fn run_geoip_check() -> SelfTestCheck {
    match augur_core::geoip::check_status() {
...
pub fn run_hf_token_check() -> SelfTestCheck {
    let path = home_relative(".cache/augur/hf_token");
    if path.as_ref().map(|p| p.exists()).unwrap_or(false) {
        ...
        SelfTestCheck {
            name: "HF token configured (optional, for diarization)".into(),
            status: CheckStatus::Skip,
            message: "not configured — `augur setup --hf-token <T>` enables \
                      speaker diarization"
                .into(),
        }
    }
...
pub fn run_full_translation_check() -> SelfTestCheck {
    use augur_translate::TranslationEngine;
...
pub fn run_self_test(full: bool) -> Result<SelfTestResult, AugurError> {
    let mut checks = vec![
    ...
    let cache_root = home_relative(".cache/augur/models").unwrap_or_else(|| PathBuf::from("."));
