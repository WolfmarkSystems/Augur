...
environments, evidence vaults). VERIFY's default code path expects
to download model weights once on first use; this guide replaces
that step with a USB-transferred bundle.

## What ships in the package

A single `verify-airgap-<preset>-<date>.tar.gz` containing:
...
- **`install.sh`** — copies the staged files into
  `~/.cache/verify/models/` on the destination machine.

The whichlang classifier ships embedded in the binary itself, so
even with no air-gap package VERIFY can still classify the 16
...
The script writes `verify-airgap-<preset>-YYYYMMDD.tar.gz` to the
...
```bash
mkdir verify-airgap
tar -xzf verify-airgap-tiny-YYYYMMDD.tar.gz -C verify-airgap
bash verify-airgap/install.sh
```

`install.sh` copies the staged files into `~/.cache/verify/models/`.
After it runs, `verify` finds every model in the cache without any
network call.

### Alternative: run from the package directly

If you'd rather not stage a copy under `~/.cache/verify/`, point
VERIFY at the unpacked package via `VERIFY_AIRGAP_PATH`:

```bash
export VERIFY_AIRGAP_PATH=$HOME/verify-airgap
verify classify --classifier-backend fasttext --text "مرحبا" --target en
```

The classifier's `ModelManager` checks `VERIFY_AIRGAP_PATH` before
attempting any network egress; if `$VERIFY_AIRGAP_PATH/lid.176.ftz`
...
verify classify --text "Hola mundo" --target en

# fastText (176 languages) — uses the air-gap LID model
verify classify --classifier-backend fasttext \
    --text "مرحبا بالعالم" --target en

# end-to-end — Whisper + NLLB + advisory notice
verify translate --input sample.wav --target en
```

If any of those fail with a network error, the corresponding
weight is missing from the cache; check `~/.cache/verify/models/`
