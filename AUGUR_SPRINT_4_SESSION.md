# AUGUR Sprint 4 — 2026-04-25 — Session log

## P1 fastText default fix: PASSED
- Default classifier: **whichlang** (was fasttext) — production
  default flip, pinned by
  `apps/augur-cli/src/main.rs::tests::default_classifier_backend_is_whichlang`.
...
- `--classifier-backend` `--help` text updated: whichlang →
  ...
  cited.
- Sprint 1 probe `crates/augur-classifier/examples/lid_label_probe.rs`
  committed under a `fasttext-probe` feature gate so it does not
  run as part of normal `cargo build` / `cargo check`. Reproduce
  with `cargo run -p augur-classifier --features fasttext-probe
...
- CLI: `augur transcribe --temperature <f32> --max-retries <u8>`.

## P3 PDF extraction: PASSED
- Added `PipelineInput::Pdf` to augur-core; `detect_input_kind`
  routes `.pdf` (case-insensitive).
- `augur_ocr::extract_pdf_text` tries `pdf-extract` (pure Rust,
  no system deps) for the text layer first; falls back to
  `pdftoppm` (poppler) rasterize → per-page Tesseract OCR for
  scanned PDFs. Missing `pdftoppm` surfaces as `AugurError::Ocr`
  with the install hint, never a panic.
- CLI auto-routes PDFs in both `augur translate --input doc.pdf`
  and `augur batch`. Strata feature build also picks up PDFs
...
- **Default-build test count: 53 unit tests passing**
  ...
  tests `#[ignore]`-gated on `AUGUR_RUN_INTEGRATION_TESTS=1`.
- **`--features augur-plugin-sdk/strata` build**: same 53 unit
